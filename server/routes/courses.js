...
router.post("/", async (req, res) => {
  try {
    const { vendor_id, code, title, level, duration, price, seats, delivery, next_start, description, badge, tags, instructor_id, delivery_location_id } = req.body;
    ...
    const { rows } = await pool.query(`
      INSERT INTO courses (vendor_id, code, title, level, duration, price, seats, enrolled, delivery, next_start, description, badge, tags, instructor_id, delivery_location_id)
      VALUES ($1,$2,$3,$4,$5,$6,$7,0,$8,$9,$10,$11,$12,$13,$14)
      RETURNING *
    `, [vendor_id, code, title, level, duration, price, seats, delivery, next_start, description, badge || '', tags || [], instructor_id || null, delivery_location_id || null]);
  ...
  try {
    ...
    await pool.query(`
      UPDATE courses SET
        ...
        tags=$12, instructor_id=$13, delivery_location_id=$14
      WHERE id=$15
    `, [vendor_id, code, title, level, duration, price, seats, delivery, next_start, description, badge || '', tags || [], instructor_id || null, delivery_location_id || null, req.params.id]);
