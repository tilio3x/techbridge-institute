...
export default function CoursesView({ enrolledCourses, onEnroll, vendors, courses }) {
  ...
  return (
    <div style={{ padding: "40px 24px", maxWidth: 1200, margin: "0 auto" }}>
      <h2 style={{ fontSize: 36, fontWeight: 900, color: "#1e293b", marginBottom: 8, fontFamily: "Georgia, serif" }}>Course Catalog</h2>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {[{ id: "all", name: "All Vendors" }, ...vendors].map(v => (
            <button key={v.id} onClick={() => setFilter(v.id)} style={{
              background: filter === v.id ? "rgba(14,165,233,0.2)" : "#f8fafc",
              color: filter === v.id ? "#0ea5e9" : "#94a3b8",
              border: filter === v.id ? "1px solid #0ea5e9" : "1px solid #e2e8f0",
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {["all", "Beginner", "Intermediate", "Advanced"].map(l => (
            <button key={l} onClick={() => setLevelFilter(l)} style={{
              background: levelFilter === l ? "rgba(99,102,241,0.2)" : "#f8fafc",
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {["all", "Online", "Hybrid", "In-Person"].map(d => (
            <button key={d} onClick={() => setDeliveryFilter(d)} style={{
              background: deliveryFilter === d ? "rgba(34,197,94,0.2)" : "#f8fafc",
